// dsh-bili-taskmaster — CLIENT (browser) half (DSH dynamic Cordis plugin)
//
// This file is the exact `code.client` function body of the running plugin
// (`bili-1`, package pkg-42). DSH's cordis-client-runner evaluates it in the
// browser as the body of an async function whose parameters ARE the client
// surface (`React`, `console`, `styles`, `host`, `harness`, …), and it must
// `return` a Cordis plugin object.
//
// Constraints the code already respects:
//   * No JSX / TypeScript / import — build everything with React.createElement.
//   * No fetch / timers / require / process / Buffer: declare
//     inject: ['timer'] and use ctx.interval / ctx.timeout; call the host via
//     host.call(method, args).
//   * CSS is injected through styles.insert and removed on unload.
//
// What the client half owns:
//   * The draggable/resizable "Bilibili 鲸鱼监工" overlay injected into the
//     shell.overlay slot.
//   * Video playback (1080p via MediaSource + MSE, lower qualities via the
//     MP4 proxy), danmaku rendering with font/density controls, lane-busy
//     anti-overlap and pause-freeze, the whale teaser danmaku, the task-done
//     celebration overlay, quality selection, favorites, auto-play-next, and
//     the account/QR-login and settings panels.
return {
  inject: ['timer'],
  apply(ctx) {
    ...
    function BiliPlayer() {
      ...
      function applyQuality(v, q) {
        ...
        if (q === 80) {
          ...
          if (v.dash && v.dash.videoUrl) {
            ...
            host.call('get-dash', { bvid: v.bvid, cid: v.cid }).then(function (d) {
              if (d && d.videoUrl) { if (v.bvid === currentBvid) startMSE(d) }
            }).catch(function () {})
          }
        } else if (q > 0) {
          host.call('get-playurl', { bvid: v.bvid, cid: v.cid, qn: q }).then(function (r) {
      ...
      async function getStatus() {
        try { return await host.call('get-status', {}) } catch (e) { return null }
      }
      ...
      async function loadDanmaku(v) {
        const bvid = v.bvid
        const d = await host.call('get-danmaku', { cid: v.cid })
      ...
      async function favVideo() {
        ...
        const r = await host.call('fav-video', { aid: video.aid })
      ...
      async function refresh(manual) {
        ...
        if (s.agentStatus === 'idle' && prevAgentStatus === 'running') {
          setCelebrate(true)
          ctx.timeout(function () { setCelebrate(false) }, 5000)
          if (autoPauseEnabled) {
      ...
      async function startLogin() {
        ...
        setQrData(null)
        try {
          const r = await host.call('login-start', {})
          if (r && r.qrcode_url) {
      ...
      function logout() {
        host.call('logout', {}).then(function (r) {
          if (r && r.video && r.video.bvid) {
            noteVideo(r.video)
          }
        }).catch(function () {})
      ...
      function onQuality(e) {
        ...
        if (q === 80) {
          ...
          if (video.dash && video.dash.videoUrl) {
            ...
            host.call('get-dash', { bvid: video.bvid, cid: video.cid }).then(function (d) {
              if (d && d.videoUrl) {
          ...
          if (q === 0 && video.playurl) {
            ...
            host.call('get-playurl', { bvid: video.bvid, cid: video.cid, qn: q }).then(function (r) {
      ...
      function onDmFont(e) {
        const v = Number(e.target.value)
        setDmFontScale(v)
        host.call('set-dmfont', { value: v }).catch(function () {})
      }

      function onDmDensity(e) {
        const v = Number(e.target.value)
        setDmDensity(v)
        host.call('set-dmdensity', { value: v }).catch(function () {})
      }
      ...
      function onAutoPause(e) {
        ...
        autoPauseEnabled = v
        setAutoPause(v)
        host.call('set-autopause', { value: v }).catch(function () {})
      }
      ...
      React.useEffect(function () {
        ...
        init()
        const iv = ctx.interval(function () { refresh(false) }, 1500)
        return function () { iv() }
      }, [])

      React.useEffect(function () {
        const iv = ctx.interval(function () {
      ...
      React.useEffect(function () {
        ...
        async function tick() {
          if (stop) return
          let r = null
          try { r = await host.call('login-poll', {}) } catch (e) { return }
          if (stop) return
          if (r.status === 'success') {
        ...
        tick()
        const iv = ctx.interval(tick, 2000)
        return function () { stop = true; iv() }
      }, [polling])
